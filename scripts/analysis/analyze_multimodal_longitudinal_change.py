# scripts/analysis/analyze_multimodal_longitudinal_change.py
...
OUTPUT_PATH = "data/analysis/multimodal_longitudinal_change.csv"
OUTPUT_CORR_PATH = "data/analysis/multimodal_delta_correlation.csv"
OUTPUT_DIR = Path("results/plots/multimodal_longitudinal_change")
...
def main():
    ...
    for feature in FEATURE_COLUMNS:
        ...
        if pre_col in wide_df.columns and post_col in wide_df.columns:
            wide_df[f"delta_{feature}"] = (
                wide_df[post_col] - wide_df[pre_col]
            )
    ...
    for feature in FEATURE_COLUMNS:
        ...
        valid_df = wide_df.dropna(subset=[delta_col, "delta_ucla_total"]).copy()

        if len(valid_df) >= 3:
            r, p = spearmanr(valid_df[delta_col], valid_df["delta_ucla_total"])
        else:
            r, p = None, None
        ...
        output_path = OUTPUT_DIR / f"{delta_col}_vs_delta_ucla.png"
    ...
    print("\n=== multimodal longitudinal change ===")
    print(wide_df)
